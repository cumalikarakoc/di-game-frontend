      ...
      <div className="rounded">
          ...
          {tables.map(table => {
            return (
              <div
                ...
                className={`p-4 border-r border-b flex-grow ${
                  table.name === selectedTableName ? "bg-grey-darker" : ""
                }`}
              >
                {table.name}
              </div>
        ...
        <div>
          <table className="w-full">
            <thead>
              <tr>
                {selectedTable.columns.map(columnName => (
                  <th className="border-1 border-b border-l p-2">
                    {columnName}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {selectedTable.rows.map(row => (
                <tr>
                  {selectedTable.columns.map(columnName => (
                    <td className="border-1 border-b border-l p-2">
                      {row[columnName]}
                    </td>
                  ))}
                </tr>
